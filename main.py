...
class IngestionAgent:
    ...
    @staticmethod
    def _is_womens_competition(league_text: str) -> bool:
        """v2026.7: 'Women' only ever shows up in the league-header line
        (e.g. 'Eredivisie, Women'), never in the team names themselves --
        confirmed against a real odibets women's fixture where both team
        names were plain club names with no gender marker at all. Any
        detection scheme that looks for a marker on the team name (what this
        file assumed one version ago) will miss every one of these."""
        return bool(league_text) and 'women' in league_text.lower()
    ...
    def _parse_card_format(raw_text: str) -> List[Dict[str, Any]]:
        ...
        v2026.7: league-header lines (e.g. 'Soccer /Netherlands /Eredivisie,
        Women(1)') used to be discarded outright as noise. They're now kept
        and attached to every fixture under that header as 'league_hint',
        specifically so a 'Women' competition can be detected downstream and
        routed away from ClubElo/Understat/API-Football -- none of which
        cover women's football, so blind fuzzy name-matching against them for
        a women's fixture risks silently returning a men's team's data. Also
        widened the header regex to match a 3-part 'Soccer /Country /League'
        path, not just the 2-part 'Country / League' shape -- the 2-part-only
        version let 'Soccer /Bulgaria /Second Prof. League(1)' fall through
        as if it were a team name.
        """
        link_re = re.compile(r'^\[(.*?)\]\(https?://\S+\)$')
        glued_odds_re = re.compile(r'1\s*(\d+\.\d{2})X\s*(\d+\.\d{2})2\s*(\d+\.\d{2})', re.I)
        league_header_re = re.compile(r"^[\w .,&'\-]+(?:/[\w .,&'\-]+){1,3}\(\d+\)$")

        lines = [l.strip() for l in raw_text.split('\n') if l.strip()]
        matches: List[Dict[str, Any]] = []
        pending_teams: List[str] = []
        current_league_hint: Optional[str] = None
        ...
        for line in lines:
            ...
            if re.match(r'^starts in', text, re.I):
                continue
            if league_header_re.match(text):
                current_league_hint = text  # kept, not discarded -- see docstring
                continue
            if '\u2022' in text:
                current_league_hint = text
                continue
            ...
            if odds_m:
                if len(pending_teams) >= 2:
                    ...
                    matches.append({
                        ...
                        "league_hint": current_league_hint,
                        "is_womens": IngestionAgent._is_womens_competition(current_league_hint),
                    })
                pending_teams = []
                continue
    ...
    def _parse_table_format(raw_text: str) -> List[Dict[str, Any]]:
        ...
        v2026.7: league-header lines are now captured as 'league_hint' on
        each subsequent fixture (same reasoning as _parse_card_format) rather
        than discarded, so a 'Women' competition can be flagged downstream.
        """
        ...
        filtered = []
        current_league_hint: Optional[str] = None
        league_hints: List[Optional[str]] = []
        for l in lines:
            if '\u2022' in l:
                current_league_hint = l
                continue
            ...
            if re.match(r'^\d{1,2}[\/\.]\d{1,2}(?:\/\d{2,4})?(\s*,\s*|\s*-\s*|\s+)\d{1,2}:\d{2}', l, re.I):
                continue
            filtered.append(l)
            league_hints.append(current_league_hint)
            ...
            if not glued_odds_full_re.match(filtered[i]) and glued_odds_full_re.match(filtered[i + 1]):
                ...
                if split and len(odds_vals) == 3:
                    ...
                    matches.append({
                        ...
                        "oA": odds_vals[2],
                        "league_hint": hint,
                        "is_womens": IngestionAgent._is_womens_competition(hint),
                    })
                i += 2
                continue
            i += 1
...
class LiveDataAgent:
    ...
    @classmethod
    async def harvest_all(cls, home_team: str, away_team: str, client: httpx.AsyncClient,
                           skip_gendered_sources: bool = False) -> Dict[str, Any]:
        """
        v2026.7: skip_gendered_sources=True routes AROUND ClubElo,
        API-Football team lookup, and Understat entirely. Those three are
        men's-football-only databases with no way to tell them "only match
        against the women's competition" -- fuzzy name search against a
        men's-only database for a women's fixture can silently return a
        men's team's rating/xG as if it were real signal for the match being
        analyzed. Safer to run the match on Shin (odds-derived, always
        correct-competition since it's your own pasted odds) alone and say so
        honestly than to guess with contaminated data.
        """
        home_norm = normalize_team_name(home_team)
        away_norm = normalize_team_name(away_team)

        if skip_gendered_sources:
            elo_h, elo_a, home_info, away_info, xg_pair = None, None, None, None, None
            consensus = await cls.fetch_bookmaker_consensus(home_norm, away_norm, client)
        else:
        ...
        provenance = {
            "elo_home": "unsupported_womens_competition" if skip_gendered_sources else ("clubelo_live" if elo_h else "default_1500_fallback"),
            "elo_away": "unsupported_womens_competition" if skip_gendered_sources else ("clubelo_live" if elo_a else "default_1500_fallback"),
        }
        ...
        elif skip_gendered_sources:
            lam = round(max(0.35, 1.35 + (elo_diff / 500.0)), 2)
            mu = round(max(0.35, 1.20 - (elo_diff / 500.0)), 2)
            xg_source = "unsupported_womens_competition"
        else:
            lam = round(max(0.35, 1.35 + (elo_diff / 500.0)), 2)
            mu = round(max(0.35, 1.20 - (elo_diff / 500.0)), 2)
            xg_source = "unavailable_elo_proxy_used"

        gendered_label = "unsupported_womens_competition"
        provenance.update({
            "injuries": gendered_label if skip_gendered_sources else ("api_football_live" if API_FOOTBALL_KEY else "unavailable_no_api_key"),
            "form": gendered_label if skip_gendered_sources else ("api_football_live" if API_FOOTBALL_KEY else "unavailable_no_api_key"),
            "h2h": gendered_label if skip_gendered_sources else ("api_football_live" if API_FOOTBALL_KEY else "unavailable_no_api_key"),
            "weather": gendered_label if skip_gendered_sources else ("open_meteo_live" if weather else "unavailable_no_venue_coords"),
...
async def execute_pipeline(req: PipelineRequest):
    ...
    async with httpx.AsyncClient() as client:
        for rm in raw_matches:
            ...
            is_womens = bool(rm.get("is_womens"))

            stats = await LiveDataAgent.harvest_all(t1, t2, client, skip_gendered_sources=is_womens)
            ...
            match_record = {
                ...
                "home_team": t1,
                "away_team": t2,
                "league_hint": rm.get("league_hint"),
                "is_womens_competition": is_womens,
                "odds": {"1": oH, "X": oD, "2": oA},
                "tactical_data": {
            ...
            if is_womens:
                # v2026.7 FIX: hard-excluded from ultra-safe regardless of what
                # the filter would otherwise say -- ClubElo/Understat/API-Football
                # were deliberately skipped for this match (see harvest_all), so
                # there's no independent real-world signal backing this pick
                # beyond the odds themselves. Never silently let it into an accumulator.
                match_record["ultra_safe"] = False
                match_record["data_quality_warning"] = (
                    "Detected as a women's-competition fixture (league header contained "
                    "'Women'). ClubElo, Understat, and API-Football have no reliable "
                    "women's-football coverage, so those sources were skipped entirely for "
                    "this match rather than risk matching the wrong (men's) team. This pick "
                    "rests on Shin (odds-derived) only and is excluded from ultra-safe "
